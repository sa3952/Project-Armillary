...
def test_benchmark_chart_full_response_shape():
    ...
    assert set(data.keys()) == {
        ...
        "derived_geometry", "derived_methods", "calculation_trace",
    }
...
def test_default_request_only_computes_core_astronomy():
    ...
    assert data["requested_options"]["include_fixed_stars"] is True
...
def test_lunar_phase_search_rejects_silent_moshier_fallback_beyond_file_range():
    payload = base_payload()
    payload["datetime"] = {
        "year": 2399,
        "month": 12,
    ...
    assert response.json()["detail"]["code"] == "full_ephemeris_required"
