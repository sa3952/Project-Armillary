...
RUN_LOCAL = PROJECT_ROOT / "scripts" / "run-local.sh"
...
def test_process_control_policy_discloses_header_and_event_boundary():
    policy = (
        PROJECT_ROOT / "docs" / "PRIVACY_LOGGING_POLICY.md"
    ).read_text(encoding="utf-8")

    assert "process-control exception重新拋出路徑" in policy
    assert "不保證補送security headers或completion event" in policy
...
def test_backend_has_no_third_party_telemetry_or_persistence_dependency():
    guard_path = PROJECT_ROOT / "scripts" / "verify_privacy_dependencies.py"
    completed = subprocess.run(
        [sys.executable, str(guard_path), "--check"],
        cwd=PROJECT_ROOT,
        check=False,
        capture_output=True,
        text=True,
