...
Sidereal 模式採「tropical 宮頭 − ayanamsa」換算（僅套用在黃道經度類的值上；
ARMC 是恆星時性質的角度，不是黃道經度，不做 ayanamsa 平移）。
...
def compute_houses(house_system_code: str, jd_ut: float, location, ctx, trace: Trace) -> dict:
    ...
    trace.add(
        f"宮位制：{name} ({code})",
        formula="swe.houses_ex(JD_UT, 地理緯度, 地理經度, 宮位制代碼)"
                + ("；sidereal 模式：黃道經度類角點一律減去 ayanamsa" if ayanamsa is not None else ""),
