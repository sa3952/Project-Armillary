...
from .core.time_utils import NonexistentLocalTimeError, compute_time_conversion
...
from .request_limits import ChartRequestBoundary
...
try:
    _PYSWISSEPH_DISTRIBUTION_VERSION = importlib.metadata.version("pyswisseph")
...
def _compute_chart_locked(req: ChartRequest):
    ...
    time_conv = compute_time_conversion(req.datetime, req.timezone, req.location, trace)
    ...
    sect = None
    if req.options.include_lots or req.options.include_void_of_course:
...
def create_app(
    ...
    *,
    ...
    application.add_exception_handler(
        ...
        _handle_nonexistent_local_time,
    )
    application.add_exception_handler(
        FullEphemerisRequiredError,
        _handle_full_ephemeris_required,
    )
    application.add_exception_handler(
        HouseSystemUnavailableError,
        _handle_house_system_unavailable,
    )
