...
GUARD = PROJECT_ROOT / "scripts" / "verification" / "verify_privacy_dependencies.py"
...
def test_privacy_dependency_guard_allows_only_reviewed_static_place_catalog_sqlite():
    guard = _load_guard()
    reviewed_path = (
        PROJECT_ROOT / "backend" / "app" / "core" / "place_catalog.py"
    )

    guard.check_python_source(reviewed_path)


def test_privacy_dependency_guard_rejects_sqlite_outside_reviewed_module(tmp_path):
    guard = _load_guard()
    source_path = tmp_path / "place_catalog.py"
    source_path.write_text("import sqlite3\n", encoding="utf-8")
