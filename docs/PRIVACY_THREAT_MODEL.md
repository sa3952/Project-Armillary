...
The application has no intended outbound network path, account/chart
database, cache, or chart persistence. It has one bundled immutable SQLite
place catalog containing public product data; the catalog is opened read-only
and does not store queries. The browser retains form state for the active page
and may create user-requested downloads.
