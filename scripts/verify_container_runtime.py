...
def _copy_build_context(destination: Path) -> Path:
    ...
    shutil.copytree(PROJECT_ROOT, context, ignore=ignore)
...
def _platform_args(platform: str | None) -> list[str]:
...
def _platform_contract(platform: str) -> tuple[str, str]:
    ...
    if len(parts) not in (2, 3) or not all(parts[:2]):
        raise GateFailure(f"unsupported Docker platform syntax: {platform}")
    operating_system, architecture = parts[:2]
    if operating_system != "linux" or architecture not in {
        "amd64",
        "arm64",
    }:
        raise GateFailure(f"unsupported production platform: {platform}")
...
def _two_worker_isolation(
    ...
    try:
        ...
        # Match, rather than exceed, the production --limit-concurrency
        # contract. Over-limit rejection belongs to the future proxy boundary;
        # this gate is proving mode isolation for accepted requests.
