(function attachViewModel(root, factory) {
  ...
  const MODULE_COVERAGE = Object.freeze({
    ...
    "derived_methods.essential_dignities": { zh: "本質尊貴", section: "dignities" },
  ...
  const PLANET_BY_KEY = Object.freeze({
  ...
  function rulerName(key) {
    if (!key) return DASH;
    const normalized = String(key).toLowerCase();
    return PLANET_BY_KEY[normalized] || SIGN_BY_KEY[normalized] || String(key);
  }
  ...
  function horizonContractNotes(value) {
    if (!value) return [];
    return [
      "上升與下降以觀測地的視地平為準，包含標準大氣折射與天體上緣；中天另列上中天與下中天。",
    ];
  }
  ...
  function housesSection(response) {
    ...
    return {
      ...
      notes: [`宮位制：${houses.system_name || houses.system_code || DASH}。`],
      tables: [{
        title: "宮始點",
        columns: ["宮", "黃經", "星座度數"],
  ...
  function dignitiesSection(response) {
    ...
    if (isEmptyModule(dignities)) {
      return absentSection(
        "dignities", "本質尊貴", RING.METHOD, "本次未請求本質尊貴。"
      );
    }
    ...
    const tables = [{
      ...
      ].map(([technique, profile]) => [technique, profile ? "已選定" : "未選定"]),
    }];
  ...
  function lunarApsidesSection(response) {
    ...
    return {
      ...
      notes: methodNotes(module).concat([
        "三個點都是月球軌道的幾何點，不是物理天體；本項顯示位置，不自動加入相位判定。",
      ]),
  ...
  function parallaxMoonSection(response) {
    ...
    return {
      ...
      notes: methodNotes(module).concat([
        ...
        + "地心參考值保留，站心值另計。",
        "整張盤以地心位置為基準；月亮另列出生地觀測者所見的站心位置，後續相關判定使用站心月亮。",
      ]),
      tables: [{
        title: "兩個月亮",
        columns: ["來源", "黃經 λ"],
  ...
  function extraAnglesSection(response) {
    ...
    return {
      ...
      status,
      notes: [
        "五個角點由Swiss Ephemeris的宮位計算附帶取得；反宿命點由宿命點的精確對蹠位置導出。這些都不是古典占星的四軸。",
      ],
      tables: [{
        title: "角點",
        ...
        rows: Object.entries(angles).map(([key, value]) => {
          ...
          return [value.zh || key, num(longitude, 6), signPosition(longitude)];
        }),
      }],
      blocks: [],
    };
    ...
    return {
      ...
      tables: [{
        ...
        rows: bodies.map((body) => {
          ...
          return [
            body.name || body.key,
            ({ rises_and_sets: "有升有降", always_above: "始終在地平線上",
              always_below: "始終在地平線下" })[body.visibility]
  ...
  function declinationAspectsSection(response) {
    ...
    return {
      ...
      status: { state: "present" },
      notes: methodNotes(module).concat([
        `這是近現代技法；本次使用 ${num(module.orb_degrees, 3)}° 容許度。`,
      ]),
      tables: [{
        title: "赤緯相位",
  ...
  function dignityProfileSections(response) {
    ...
    return Object.keys(TECHNIQUE_LABELS).map((technique) => {
      ...
      const tables = [{
        ...
        rows: objects.map((object) => (technique === "domicile_exaltation"
          ...
          : [
            object.name || object.key, signName(object.sign),
            num(object.degree_in_sign, 4),
            rulerName(object.ruler_name || object.ruler),
      ...
      Object.values(results).forEach((other) => {
        ...
        const sourceName = String(other.source || "其他來源").split(",")[0];
        tables.push({
          title: `並列比較（不影響選定）：${sourceName}`,
          columns: ["天體", "星座", "守護"],
    ...
    return {
      header: {
        ...
        warnings: warnings.filter(
          (warning) => warning.code !== "provisional_method_result"
        ).map((warning) => ({
